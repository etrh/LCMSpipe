...
On UNIX-based systems you will first need to ensure that the following requirements are installed:  
  `sudo apt-get install -y libxml2-dev libcurl4-openssl-dev libssl-dev netcdf-bin libnetcdf-dev gfortran libblas-dev liblapack-dev libpcre++-dev liblzma-dev libbz2-dev libgtk2.0-dev bwidget libglu1-mesa-dev freeglut3-dev mesa-common-dev default-jre default-jdk libmariadbclient-dev libmariadb-client-lgpl-dev`
...
group_colors <- brewer.pal(3, "Set1")[1:2] ## Define colors for the groups in the data
names(group_colors) <- c("Rik", "WT")

dir.create("QC")
png("QC/01 - BPC.png", width = 7, height = 7, units = "in", res = 600)
...
png("QC/02 - TIC.png", width = 7, height = 7, units = "in", res = 600)
...
png("QC/08 - Peak_Density_specific_mz_rt_window.png", 
    width = 15, height = 7, units = "in", res = 1200)
plotChromPeakDensity(xdata, mz = c(200,300), rt = c(50,200))
dev.off()
...
png("QC/14 - peak_density_correspondence.png", 
    width = 15, height = 7, units = "in", res = 1200)
...
write.csv(metaboanalyst_dat, file='PeakIntensityMatrix_MetaboAnalyst.csv')
...
#Verify grouping
xsaC <- groupCorr(xsaF) # This function calculates the pearson correlation coefficient based on the peak shapes of every peak in the pseudospectrum to separate co-eluted substances. Run groupFWHM before groupCorr to avoid extremely long running times
...
rule.table.file1 <- system.file('rules/primary_adducts_pos.csv', package = "CAMERA")
rule.table.file2 <- system.file('rules/primary_adducts_neg.csv', package = "CAMERA")
rule.table.file3 <- system.file('rules/extended_adducts_pos.csv', package = "CAMERA")
rule.table.file4 <- system.file('rules/extended_adducts_neg.csv', package = "CAMERA")
```
...
```{r Mixture Model Analysis and Normalization with metabomxtr}
...
test_model_full <- ~ as.factor(phenotype) | as.factor(phenotype) + as.character(SampleNames) + as.character(`Acquisition Time`) + 
...
metabolite_names <- names(metabomxtr_table_complete)[11:length(metabomxtr_table_complete)] ### length(metabomxtr_table_complete) should be most probably changed to a much smaller number. It does not seem to make any sense to use all detected features. Perhaps we should keep only those that are truly significant? It also seems that a high number of metabolites causes an error (or maybe the error caused by something else)

test_run_full <- mxtrmod(ynames = metabolite_names, 
                         mxtrModel = test_model, data = metabomxtr_table_complete)

test_run_reduced <- mxtrmod(ynames = metabolite_names, mxtrModel = test_model_reduced,
                            data = metabomxtr_table_complete, fullModel = test_model_full)

mxtrmodLRT(test_run_full, test_run_reduced, adj = "BH")
```