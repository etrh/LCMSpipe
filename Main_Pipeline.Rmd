...
metaboanalyst_df <- rbind(c("Replicates", rep("4ul", len_df-1)),metaboanalyst_df) # sample volume row / 
#injection_vol <- sample_info_extracted[,"Inj Vol (µl)", drop = F][rownames(sample_info_extracted[,"Inj Vol (µl)", drop = F]) %in% #names(metaboanalyst_df)[-1],]
#injection_vol <- injection_vol[ order(names(metaboanalyst_df)[-1]), ]
#rbind(c("Replicates", injection_vol),metaboanalyst_df)
