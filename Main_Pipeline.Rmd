...
```{r Dealing with missing values, and mixture model analysis and normalization with metabomxtr}
...
# a better option would be something like this: metabolite_names <- names(which(colSums(is.na(metabomxtr_table_complete)) >= 5))
...
first_phenotype <- round(exp(fullModelResults$xInt + fullModelResults$x_phenotypeFrozen) / 
                        (1 + exp(fullModelResults$xInt + fullModelResults$x_phenotypeFrozen)), digits = 2)
...
first_phenotype.mean <- round(fullModelResults$zInt+fullModelResults$z_phenotypeFrozen,digits=2)
second_phenotype.mean <- round(fullModelResults$zInt,digits=2)
phen.mean.diff <- round(fullModelResults$z_phenotypeFrozen,digits=2)
