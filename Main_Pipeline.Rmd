...
for(i in random_peak_idx){
    ...
    for(i in 1:length(mzml.files)){
    
    chr_raw <- chromatogram(raw_data %>% filterFile(i), mz = mzr, rt = rtr)
    
    plot(chr_raw, col = group_colors[chr_raw$sample_group], lwd = 2)
    highlightChromPeaks(type = "polygon", xdata %>% filterFile(i), col = group_colors[chr_raw$sample_group], lty = 3, rt = rtr, mz = mzr)
