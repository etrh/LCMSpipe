...
for(i in length(sample_info_extracted)){
  names(sample_info_extracted)[i] <- sample_info_extracted[[i]]$Value[1]
}

## Job/experiment information/metadata

worklist_pos <- list.files(path = xml.path, pattern = "Worklist.xml", 
                              all.files = T, full.names = T, recursive = T)
...
experiment_metadata <- lapply(worklist_xml, function(f){
data.frame(
...
### worklist table (experiment_metadata) doesn't seem to be useful and consistent.. The order of running samples in each file is different!
```
...
metabomxtr_table[,-1] <- log2(metabomxtr_table[,-1])
