...
```{r Requirements, eval=FALSE, include=FALSE}
...
for(i in random_peak_idx){
    
    mzr <- c(chromPeaks(xdata)[i,"mzmin"] - 0.005, chromPeaks(xdata)[i,"mzmax"] + 0.005)
    rtr <- c(chromPeaks(xdata)[i,"rtmin"] %>% floor - 50, chromPeaks(xdata)[i,"rtmax"] %>% ceiling + 50)
    chromPeaks(xdata, mz = mzr, rt = rtr) -> infoPeaks
    
    png(paste0("QC/09 - EIC/09 - Identified_peaks_in_mz_", mzr[1], "-", mzr[2], "_rt_", rtr[1], "-", rtr[2], ".png"), 
        width = 15, height = 11, unit = "in", res = 300)
    par(mfrow=c(n2mfrow(length(mzml.files))))
    
    for(j in 1:length(mzml.files)){
    
    chr_raw <- chromatogram(raw_data %>% filterFile(j), mz = mzr, rt = rtr)
    
    plot(chr_raw, col = group_colors[chr_raw$sample_group], lwd = 2, 
         sub = paste0("SNR = ", paste((infoPeaks[which(infoPeaks[,"sample"] == j),"sn"]), collapse=" | ")))
    highlightChromPeaks(type = "polygon", xdata %>% filterFile(j), col = group_colors[chr_raw$sample_group], lty = 3, rt = rtr, mz = mzr)
...
pdf(file = "QC/Diagnostic_Plots.PDF", onefile = TRUE)
plotQC(xset_conv)
dev.off()
...
rownames(stat.res$plsda_vip.csv) <- stat.res$plsda_vip.csv[,1]
stat.res$plsda_vip.csv <- stat.res$plsda_vip.csv[,-1]

rownames(stat.res$splsda_loadings.csv) <- stat.res$splsda_loadings.csv[,1]
stat.res$splsda_loadings.csv <- stat.res$splsda_loadings.csv[,-1]

rownames(stat.res$volcano.csv) <- stat.res$volcano.csv[,1]
stat.res$volcano.csv <- stat.res$volcano.csv[,-1]

pls.res <- stat.res$plsda_vip.csv[which(abs(stat.res$plsda_vip.csv$`Comp. 1`) > 4.0 | abs(stat.res$plsda_vip.csv$`Comp. 2`) > 4.0),]
spls.res <- stat.res$splsda_loadings.csv[which(abs(stat.res$splsda_loadings.csv$`comp 1`) > 0.4 | abs(stat.res$splsda_loadings.csv$`comp 2`) > 0.4),]

top.features <- list(PLSDA = pls.res, SPLSDA = spls.res, Volcano = stat.res$volcano.csv)

top.features.intensity <- lapply(top.features, function(x){merge(x, metaboanalyst_dat[-1,] %>% as.data.frame, by = "row.names")})
top.features.intensity <- lapply(top.features.intensity, function(x){dplyr::rename(x, mz.rt = Row.names)})

top.features.intensity$PLSDA[ , !names(top.features.intensity$PLSDA) %in% paste0("Comp. ", 3:8)] -> top.features.intensity$PLSDA
top.features.intensity$SPLSDA[ , !names(top.features.intensity$SPLSDA) %in% paste0("comp ", 3:8)] -> top.features.intensity$SPLSDA

for(i in 4:length(top.features.intensity$PLSDA))
{
  top.features.intensity$PLSDA[,i] %<>% as.character %>% as.numeric
}

for(i in 4:length(top.features.intensity$SPLSDA))
{
  top.features.intensity$SPLSDA[,i] %<>% as.character %>% as.numeric
}

for(i in 6:length(top.features.intensity$Volcano))
{
  top.features.intensity$Volcano[,i] %<>% as.character %>% as.numeric
}

DFPLS <- top.features.intensity$PLSDA[,4:length(top.features.intensity$PLSDA)]
top.features.intensity$PLSDA$most.intense <- colnames(DFPLS)[apply(DFPLS,1,which.max)]

DFSPLS <- top.features.intensity$SPLSDA[,4:length(top.features.intensity$SPLSDA)]
top.features.intensity$SPLSDA$most.intense <- colnames(DFSPLS)[apply(DFSPLS,1,which.max)]

DFVOL <- top.features.intensity$Volcano[,6:length(top.features.intensity$Volcano)]
top.features.intensity$Volcano$most.intense <- colnames(DFVOL)[apply(DFVOL,1,which.max)]

top.features.intensity$PLSDA$Stat.Method <- rep("PLSDA", top.features.intensity$PLSDA %>% nrow)
top.features.intensity$SPLSDA$Stat.Method <- rep("SPLSDA", top.features.intensity$SPLSDA %>% nrow)
top.features.intensity$Volcano$Stat.Method <- rep("Volcano", top.features.intensity$Volcano %>% nrow)

dupl.features <- lapply(top.features.intensity, function(x) (x$mz.rt)) %>% unlist %>% table > 1
dupl.features <- which(dupl.features == T)
dupl.features

top.features.intensity <- lapply(top.features.intensity, function(x){
  cbind(
    stringr::str_split_fixed(x$mz.rt %>% stringr::str_replace_all("M", ""), "T", 2) %>% as.data.frame() %>% dplyr::rename(mz = V1, rt.seconds = V2),
    x
  )
})

### Evaluate significant peaks
dir.create("SignificantPeaks")

uniq.peaks <- lapply(top.features.intensity, function(x) (x$mz.rt)) %>% unlist %>% unique
uniq.peaks.mzrt <- stringr::str_split_fixed(uniq.peaks %>% stringr::str_replace_all("M", ""), "T", 2) %>% as.data.frame() %>% dplyr::rename(mz = V1, rt.seconds = V2)

uniq.features <- cbind(uniq.peaks,uniq.peaks.mzrt) %>% dplyr::rename(ID = uniq.peaks)
uniq.features$mz <- as.numeric(as.character(uniq.features$mz))
uniq.features$rt.seconds <- as.numeric(as.character(uniq.features$rt.seconds))

for(i in 1:nrow(uniq.features)){
  thisMZ <- c(uniq.features$mz[i] - 0.1, uniq.features$mz[i] + 0.1)
  thisRT <- c(uniq.features$rt.seconds[i] - 5, uniq.features$rt.seconds[i] + 5)
  thisID <- uniq.features$ID[i]
  
  chr_raw <- chromatogram(raw_data, mz = thisMZ, rt = thisRT)
  
  png(paste0("SignificantPeaks/", thisID, ".png"), width = 7, height = 7, units = "in", res = 400)
  
  plot(chr_raw, col = group_colors[chr_raw$sample_group], main = paste0("EIC. m/z: ", thisMZ[1], "-", thisMZ[2], " | RT: ", thisRT[1], "-", thisRT[2]))
  legend("topleft", col = group_colors, legend = names(group_colors), lty = 1)
  
  dev.off()
}

### Now manually inspect the generated plots and remove those plots that show noisy/nonsensical peaks and then run the following command
### This code will remove those peaks that you delete from the SignificantPeaks directory
top.features.processed <- lapply(top.features.intensity, 
                                 function(x){
                                   x[which(x$mz.rt %in% tools::file_path_sans_ext(dir("SignificantPeaks/"))),]
                                   }
                                 )


### Write the result to file
sink('SignificantPeaks/_significant_features_detailed.csv')

cat('WARNING: these features were detected as significantly different by more than just one method:,', 
    paste(dupl.features %>% names, collapse = ", "))

cat('\n')
cat('\n')

for(i in 1:length(top.features.processed)){
    cat('Features by ', names(top.features.processed)[i])
    cat('\n')
    write.csv(top.features.intensity[[i]])
...
peakList_QC_nofill <- read.csv("QC_HumanPlasma_3batches_with_QC/batchCorr_Data/transposed_PeakIntensityMatrix_MetaboAnalyst_withQCsamples.csv", header = T)
peakList_QC_imputed <- read.csv("QC_HumanPlasma_3batches_with_QC/batchCorr_Data/transposed_SVD_IMPUTED_PeakIntensityMatrix_MetaboAnalyst_withQCsamples.csv", header = T)
metadata_humanPlasma <- read.csv("QC_HumanPlasma_3batches_with_QC/batchCorr_Data/Metadata.csv", header = T)
peakList_QC_imputed$X -> rownames(peakList_QC_imputed)
peakList_QC_imputed[,-1] -> peakList_QC_imputed
peakList_QC_nofill$X -> rownames(peakList_QC_nofill)
peakList_QC_nofill[,-1] -> peakList_QC_nofill
sub("$", "+", peakList_QC_imputed %>% rownames()) -> rownames(peakList_QC_imputed)
peakList_QC_nofill[match((metadata_humanPlasma$X), rownames(peakList_QC_nofill)), ] -> peakList_QC_nofill_sorted
peakList_QC_imputed[match((metadata_humanPlasma$X), rownames(peakList_QC_imputed)), ] -> peakList_QC_imputed_sorted

## Perform batch alignment

# Extract peakinfo (i.e. m/z and rt of features)
peakIn <- peakInfo(PT = peakList_QC_nofill_sorted, sep = 'T', timepos = 2, start = 2)

# Perform multi-batch alignment
alignBat <- alignBatches(peakInfo = peakIn, PeakTabNoFill = peakList_QC_nofill_sorted, 
                         PeakTabFilled = peakList_QC_imputed_sorted, batches = metadata_humanPlasma$Batch, 
                         sampleGroups = metadata_humanPlasma$Group, selectGroup = 'QC')
# Extract new peak table
PT <- alignBat$PTalign

# Batch A
batchB <- getBatch(peakTable = PT, meta = metadata_humanPlasma, batch = metadata_humanPlasma$Batch, select = 'A')
BCorr <- correctDrift(peakTable = batchB$peakTable, injections = batchB$meta$InjOrder, sampleGroups = batchB$meta$Group, QCID = 'QC', G = seq(5,35,by=3))

# Batch B
batchF <- getBatch(peakTable = PT, meta = metadata_humanPlasma, batch = metadata_humanPlasma$Batch, select = 'B')
FCorr <- correctDrift(peakTable = batchF$peakTable, injections = batchF$meta$InjOrder, sampleGroups = batchF$meta$Group, QCID = 'QC', G = seq(5,35,by=3))

# Batch C
batchH <- getBatch(peakTable = PT, meta = metadata_humanPlasma, batch = metadata_humanPlasma$Batch, select = 'C')
HCorr <- correctDrift(peakTable = batchH$peakTable, injections = batchH$meta$InjOrder, sampleGroups = batchH$meta$Group, QCID = 'QC', G = seq(5,35,by=3))


# Merge drift-corrected data and normalize
mergedData <- mergeBatches(list(BCorr,FCorr,HCorr))

normData_samplePop <- normalizeBatches(peakTable = mergedData$peakTable, 
                                       batches = metadata_humanPlasma$Batch, 
                                       sampleGroup = metadata_humanPlasma$Group, population = 'Sample')

normData_allPopu <- normalizeBatches(peakTable = mergedData$peakTable, 
                                       batches = metadata_humanPlasma$Batch, 
                                       sampleGroup = metadata_humanPlasma$Group, population = 'all')

PTnorm_samplePopulation <- normData_samplePop$peakTable
PTnorm_allPopulation <- normData_allPopu$peakTable

if(identical(rownames(PTnorm_allPopulation), as.character(metadata_humanPlasma$X))) {
  metaboanalyst_PTnorm_allPopulation <- cbind(Group = interaction(metadata_humanPlasma$Group, metadata_humanPlasma$Batch), PTnorm_allPopulation)
}

if(identical(rownames(PTnorm_samplePopulation), as.character(metadata_humanPlasma$X))) {
  metaboanalyst_PTnorm_samplePopulation <- cbind(Group = interaction(metadata_humanPlasma$Group, metadata_humanPlasma$Batch), PTnorm_samplePopulation)
}

write.csv(x = metaboanalyst_PTnorm_allPopulation, "batch_corrected_metaboanalyst_over_all_population.csv")
write.csv(x = metaboanalyst_PTnorm_samplePopulation, "batch_corrected_metaboanalyst_over_Sample_population.csv")
```

```{r BatchCorrMetabolomics}
batchCorrectedPeaks <- BatchCorrMetabolomics::doBC(Xvec = t(peakList_QC_nofill_sorted), 
                            ref.idx = as.numeric(rownames(metadata_humanPlasma[which(metadata_humanPlasma$Group == "QC"),])), 
                            batch.idx = metadata_humanPlasma$Batch %>% as.factor %>% as.numeric, 
                            seq.idx = metadata_humanPlasma$InjOrder, 
                            method = "lm", 
                            minBsamp = 3)
```
...
lookup_IDs <- pheno_data %>% 
  dplyr::filter(!sample_group %in% c("QC IS", "QC Plasma")) %>% 
  select("sample_name") %>% 
  c %>% 
  unlist() %>% 
  basename %>% substr(., 1, 9) %>% 
  paste(collapse = "','")
query <- paste0("SELECT ID,GENDER,STRAIN,ZYGOSITY,LITTER_ID,EXPORTED FROM SPECIMEN AS S WHERE S.ID IN ('", lookup_IDs, "')")
...
df = data.frame(tsne::tsne(X = t %>% na.omit %>% t))
colnames(df) = c("tSNE1","tSNE2")
df$grouping = labels.plot
ggplot(df, aes(x=tSNE1,y=tSNE2,colour=grouping)) + geom_point() + theme_classic() + scale_colour_manual(values=sample_colors)
```
