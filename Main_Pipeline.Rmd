...
- Convert .d folders (Agilent proprietary format) to mzML using MSConvert (ProteoWizard toolbox)
- Use XCMS to analyze the data (pre-processing and univariate statistical analysis)  
- Use MetaboAnalyst for multivariate data analysis  
...
On UNIX-based systems you will first need to ensure the following are installed:  
  ...
  gtk  
  jre/jvm  
  `sudo apt-get install -y libxml2-dev libcurl4-openssl-dev libssl-dev netcdf-bin libnetcdf-dev gfortran libblas-dev liblapack-dev libpcre++-dev liblzma-dev libbz2-dev libgtk2.0-dev bwidget libglu1-mesa-dev freeglut3-dev mesa-common-dev`
...
pacman::p_install("MASS", "mzR", "rgl", "splines", "parallel","doParallel", "foreach", "iterators", "snow", "ROCR", "ROCS", "e1071", "randomForest", "gbm" , "rJava", "RColorBrewer", "plotly", "multtest", "XML", "caTools", "bitops", "ptw", "gplots", "tcltk2", "IPO", "CAMERA", "pcaMethods","pca3d", "Rtsne", "pander", "imager")

if(!require(xcms)){biocLite("MSnbase")}
if(!require(xcms)){biocLite("xcms")}
...
```{r Identification through MoNa MassBank}

### to be implemented
identified <- jsonlite::fromJSON("http://mona.fiehnlab.ucdavis.edu/rest/spectra/search?page=0&query=compound.metaData%3Dq%3D%27name%3D%3D%22total+exact+mass%22+and+value+%3E%3D+245+and+value+%3C%3D+255%27+and+(tags.text%3D%3D%22LC-MS%22)+and+(metaData%3Dq%3D%27name%3D%3D%22ionization+mode%22+and+value%3D%3D%22positive%22%27)+and+(metaData%3Dq%3D%27name%3D%3D%22ms+level%22+and+value%3D%3D%22MS1%22%27)&size=30&text=")
